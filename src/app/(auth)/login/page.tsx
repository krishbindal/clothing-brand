...
function LoginForm() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    setIsLoading(false)
    if (result?.error) {
      setAuthError(result.error === 'EMAIL_NOT_VERIFIED' ? 'Please verify your email address before signing in.' : 'Invalid email or password. Please try again.')
    } else if (result?.ok) {
    ...
    <div className="min-h-screen bg-brand-black flex items-center justify-center px-4 pt-20">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
        ...
        <div className="card-dark p-8 space-y-6">
          ...
          <button onClick={() => signIn('google', { callbackUrl })} className="w-full flex items-center justify-center gap-3 btn-secondary py-3">
